...
def deepallo_dataset_split(batch_size, y_type='float32'):
    ...
    train_processed_path = './data/holo-apo-chain/train_processed.npy'
    val_processed_path = './data/holo-apo-chain/val_processed.npy'
    test_processed_path = './data/holo-apo-chain/test_processed.npy'

    train_processed = np.load(train_processed_path, allow_pickle=True)
    val_processed = np.load(val_processed_path, allow_pickle=True)
    test_processed = np.load(test_processed_path, allow_pickle=True)
    
    trainset = PreprocessedDataLoader(train_processed, batch_size, shuffle=True)
    evalset = PreprocessedDataLoader(val_processed, batch_size)
