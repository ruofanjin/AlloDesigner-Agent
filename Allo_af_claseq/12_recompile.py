...
class SequenceRecompiler:
    ...
    def load_raw_votes_data(self) -> bool:
        ...
        try:
            ...
            print(f"Loaded raw votes data from {self.raw_votes_json}")
            return True
        except Exception as e:
    ...
    def get_bin_headers(self, bin_numbers: Optional[List[int]] = None) -> List[str]:
        ...
        print(f"Found {len(bin_headers)} sequences in specified bins")
        return bin_headers
    ...
    def compile_sequences(self, bin_headers: List[str], output_file: str) -> None:
        ...
        print(f"Compiled {len(bin_seqs)} sequences to {output_file}")
    ...
    def create_control_file(self, bin_headers: List[str], output_file: str) -> None:
        ...
        print(f"Created control file with {len(selected_items)} random sequences")
    ...
    def plot_sequence_vote_ratios(
        ...
        if len(bin_headers) > limit_sequences:
            print(f"Limiting plot to {limit_sequences} sequences out of {len(bin_headers)}")
        ...
        print(f"Created vote distribution plot for bin {target_bin}")
    ...
    def process_combined_bins(self) -> None:
        ...
        print(f"Processing combined bins as {bin_name}")
        
        # Get headers for all specified bins
        print(f"Extracting headers for combined bins")
        ...
        print(f"Compiling sequences for prediction")
        ...
        print(f"Creating control prediction file")
        ...
        print(f"Completed processing combined bins")
    ...
    def process_individual_bins(self) -> None:
        ...
        if len(self.bin_numbers) == 1:
            ...
            print(f"Extracting headers for bin {bin_number}")
            ...
            #     print(f"Creating vote distribution plot for bin {bin_number}")
            #     try:
            ...
            print(f"Completed processing bin {bin_number}")
        else:
    ...
    def recompile_sequences(self) -> Dict[str, str]:
        ...
        print("Completed recompilation for all bins")
        return self.dirs
...
base_output_dir = Path('./AF_ClaSeq/case/PCKS9/4ne9/run/04_recompile')
base_output_dir.mkdir(exist_ok=True)

# Determine input MSA
source_msa = './AF_ClaSeq/case/PCKS9/4ne9/run/01_iterative_shuffling/combined_filtered_all_iterations_threshold5.a3m'
...
criterion_output_dir = Path('./AF_ClaSeq/case/PCKS9/4ne9/run/04_recompile/pocket_min_distance')
criterion_output_dir.mkdir(exist_ok=True)

# Get voting results for this criterion
voting_results = './AF_ClaSeq/case/PCKS9/4ne9/run/03_voting/pocket_distance_bin20_min_0_max_20/voting_results.csv'
raw_votes_json ='./AF_ClaSeq/case/PCKS9/4ne9/run/03_voting/pocket_distance_bin20_min_0_max_20/raw_sequence_votes.json'
...
recompiler = SequenceRecompiler(
    ...
    default_pdb='./AF_ClaSeq/case/PCKS9/4ne9/default/2P4E_MODEL_0.pdb',
    voting_results=voting_results,
    bin_numbers=bin_numbers,
    num_total_bins=20,
    initial_color='#b985ba',
    combine_bins=True,  
    raw_votes_json=raw_votes_json,
)
